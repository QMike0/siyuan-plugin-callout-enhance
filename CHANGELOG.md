...
- 🇨🇳 Callout 类型支持重命名（past label 保留旧样式解析）、删除（墓碑占用）与「关于 → 清理历史数据」全库合并
- 🇨🇳 设置：past label 只读展示、墓碑二次确认、删除前 SQL 计数提示；清理使用设置窗内未保存的类型草稿
- 🇨🇳 清理顺序：先 past label→当前 label，再墓碑孤儿→NOTE；支持取消、只读工作区拒绝、保存文档提示
- 🇺🇸 Callout types: rename with past labels, delete with tombstone occupancy, and About → clean up legacy data
- 🇺🇸 Settings: read-only past label row, tombstone reclaim confirm, delete counts; cleanup respects unsaved type draft in the dialog
- 🇺🇸 Cleanup order: past label→label first, then tombstone orphans→NOTE; cancel, read-only guard, save-documents hint
