...
- **重命名 label**：未执行清理前，文档块仍保留旧 `data-subtype`；插件通过 *historical label* 继续匹配样式（不参与补全）。
- **删除类型**：块保留原 subtype，仅失去插件样式；label 记入 *墓碑*，再次使用同名需二次确认。
- **清理历史数据**（设置 → 关于）：先将 historical 对应块合并为各类型的当前 label，再把墓碑孤儿统一为 NOTE，并清空 historical 与墓碑。清理前请保存全部已打开文档，并保持思源与本设置窗口开启直至结束。
