...
// build/generate-callout-defaults-entry.ts
...
function maskUrlFromSymbolMarkup(viewBox, innerHtml) {
...
function pluginSymbolToMaskUrl(id) {
  ...
  if (!innerMatch)
    return null;
  return maskUrlFromSymbolMarkup((viewBoxMatch == null ? void 0 : viewBoxMatch[1]) || "0 0 24 24", innerMatch[1]);
}
...
function withLayoutFieldDefaults(specs) {
...
var CALLOUT_LAYOUT_FIELD_SPECS = [
  ...
  { varName: "--callout-fold-duration", label: "Fold/expand animation duration", group: "Fold", kind: "time", unit: "ms", step: 10, min: 0, max: 2e3 }
];
var CALLOUT_LAYOUT_FIELDS = withLayoutFieldDefaults(CALLOUT_LAYOUT_FIELD_SPECS);
...
var BUILTIN_LABEL_COLOR_VAR = BUILTIN_CALLOUT_PALETTE.reduce(
  (record, entry) => {
...
function buildCalloutPaletteStylesheet() {
  const decls = [
...
function buildCalloutStaticStylesheet() {
  return [buildCalloutPaletteStylesheet(), buildDefaultCalloutTypesStylesheet()].filter(Boolean).join("\n");
}

// build/generate-callout-defaults-entry.ts
var header = "/* Generated by build/generate-callout-defaults.cjs \u2014 do not edit */\n";
var css = buildCalloutStaticStylesheet();
